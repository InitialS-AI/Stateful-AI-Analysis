...
if __name__ == '__main__':
    ...
    with open('./file/cache/yelp/yelp_nosep/best_split.txt', 'r') as f:
        for line in f.readlines():
            key = tuple([int(v.strip()) for v in line.split(':')[0][1:-1].split(',')])
            best_split[key] = int(line.split(':')[1].strip().strip('\n'))

    max_gap = 0
    for (mining_support_rate, min_pattern_size) in [(40, 1), (40, 2), (40, 3), (40, 4), (40, 5),
                                                    (50, 1), (50, 2), (50, 3), (50, 4), (50, 5),
                                                    (60, 1), (60, 2), (60, 3), (60, 4), (60, 5),]:
        with open('./file/cache/yelp/yelp_nosep/mined_results_%d_%d_%d.txt' % (mining_support_rate, min_pattern_size + 1, max_gap), 'r') as f:
            mined_patterns = f.readlines()
        mined_patterns = [[int(p.strip()) for p in v.strip('\n')[1:-1].split(')')[0][1:].split(',')] for v in mined_patterns]
        topk = best_split[(max_gap, mining_support_rate, min_pattern_size + 1)]
        success_fixed = set()
        failed_fixed = set()
        for i in range(len(test_trace)):
            ...
            for j in range(topk):
                ...
                search_result = find_similar_pattern(trace, mined_patterns[j], max_gap)
                if search_result:
                    fix_pos = []
