...
if __name__ == '__main__':
    train_data = joblib.load('./file/data/train_new.data')
    test_data = joblib.load('./file/data/test_new.data')
    ...
    for i in range(len(train_trace)):
        if train_seq_labels[i][-1] != train_gt[i]:
            train_faults += [i]
    os.makedirs('./file/cache/new/', exist_ok=True)

    with open('./file/cache/new/train_trace.txt', 'w') as f:
        for fault in train_faults:
            trace = train_trace[fault]
            tmp = [str(v) + ' -1' for v in trace]
            tmp = ' '.join(tmp) + ' -2\n'
            f.writelines(tmp)
    with open('./file/cache/new/train_trace_unique.txt', 'w') as f:
        for fault in train_faults:
            trace = train_trace[fault]
    ...
    mining_support_rate = 40
    command = 'java -jar ./file/java/spmf.jar run BIDE+ ./file/cache/new/train_trace.txt'
    command += ' ./file/cache/new/mined_%d_bide.txt %d%%' % (mining_support_rate, mining_support_rate)
    os.system(command)
    ...
    # with open('./file/cache/new/mined_tks_top50.txt', 'r') as f:
    with open('./file/cache/new/mined_%d_bide.txt' % (mining_support_rate), 'r') as f:
