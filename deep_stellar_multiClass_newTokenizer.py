...
from data.data_utils import get_agnews_test_dataframe, get_agnews_train_dataframe, text_simple
from data.data_utils import get_yelp_multi_train_dataframe, get_yelp_multi_test_dataframe
from model.simple_rnn import SimpleGRUMultiClassification
from abstraction.profiling import DeepStellar

# nohup python3 -u deep_stellar_multiClass_newTokenizer.py --reprofiling True --dataset_name yelp_multi --checkpoint ./file/checkpoints/yelp_multi_newToken_ckpt_best.pth --ds_batch_size 4096 > deep_multiclass_yelp_multi.log --out_path ./file/profile/yelp_multi 2>&1 &
...
parser.add_argument('--out_path', dest='out_path', default='./file/profile/agnews_newToken/', help='output path')
parser.add_argument('--checkpoint', dest='checkpoint', default='./file/checkpoints/agnews_newToken_ckpt_best.pth', help='checkpoint')
...
if __name__ == '__main__':
    ...
    if args.dataset_name == "agnews":
        ...
        y_key = "stars"
        target_size = 5
    vocab_size, tokenizer_pipeline, tokenizer, vocab = load_tokenizer(None, args.dataset_name, args.cache_file, top_word_num=args.vocab_size)

    train_dataset = text_simple(tokenizer=tokenizer_pipeline, x_col=X_train_all[x_key].to_numpy(dtype=str), y_col=X_train_all[y_key].to_numpy())
    ...
    if os.path.exists(pca_data_path) and not args.reprofiling:
        ...
        print("Begin profiling training data")
        state_vec = []
        embedding = []
        text = []
        label = []
        ...
        for batch in (train_dataloader):
            ...
            for i in range(batch_size):
                ...
                if len(embedding_) > max_seq_length:
                    max_seq_length = len(embedding_)
                ...
                prediction_ = np.argmax(prediction_, axis=-1)
