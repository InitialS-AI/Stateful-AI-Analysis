"""
Name   : sequential_mining_yelp_nosep_unique.py
Author : Zhijie Wang
Time   : 2021/8/8
"""
...
if __name__ == '__main__':
    ...
    train_faults = []
    log_dir = './file/cache/yelp_nosep_unique/log/'
    ...
    for i in range(len(train_trace)):
        if train_seq_labels[i][-1] != train_gt[i]:
            train_faults += [i]
    os.makedirs('./file/cache/yelp_nosep_unique/', exist_ok=True)

    with open('./file/cache/yelp_nosep_unique/fault_trace.txt', 'w') as f:
    ...
    with open('./file/cache/yelp_nosep_unique/correct_trace.txt', 'w') as f:
        for i in range(len(train_trace)):
    ...
    for maxgap in [0, 1, 2, 3, 4, 5]:
        for mining_support_rate in [40, 50, 60]:
            command = 'java -jar ./file/java/spmf.jar run NOSEP ./file/cache/yelp_nosep_unique/fault_trace.txt'
            command += ' ./file/cache/yelp_nosep_unique/mined_%d_nosep_fault.txt 1 20 0 %d %d' % (
                mining_support_rate, maxgap, int(len(train_faults) * mining_support_rate / 100))
            os.system(command)

            command = 'java -jar ./file/java/spmf.jar run NOSEP ./file/cache/yelp_nosep_unique/correct_trace.txt'
            command += ' ./file/cache/yelp_nosep_unique/mined_%d_nosep.txt 1 20 0 %d %d' % (
            mining_support_rate, maxgap, int((len(train_trace) - len(train_faults)) * mining_support_rate / 100))
            os.system(command)
            ...
            for min_pattern_size in [1, 2, 3, 4, 5]:
                with open('./file/cache/yelp_nosep_unique/mined_%d_nosep.txt' % (mining_support_rate), 'r') as f:
                ...
                with open('./file/cache/yelp_nosep_unique/mined_%d_nosep_fault.txt' % (mining_support_rate), 'r') as f:
                    mined_data = f.readlines()
                mined_patterns = {}
                for line in mined_data:
                    line_tmp = line.split('#')
                ...
                with open('./file/cache/yelp_nosep_unique/mined_results_%d_%d.txt' % (mining_support_rate, min_pattern_size + 1),
                          'w') as f:
