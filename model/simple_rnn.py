...
class HumanActivityGRU(nn.Module):
    def __init__(self, input_size, rnn_size, dense_hidden_dim=None, dropout_prob=0.3, num_layers=1, target_size=12):
        super(HumanActivityGRU, self).__init__()
        self.rnn_size = rnn_size
        self.num_layers = num_layers
        self.target_size = target_size
        self.dropoutProb = dropout_prob
        self.input_size = input_size
        self.lstm = nn.GRU(input_size, rnn_size, num_layers, dropout=dropout_prob, batch_first=True)
...
class SimpleGRUMultiClassification(nn.Module):
    ...
    def profile(self, x):
        with torch.no_grad():
            ...
            lstm_out, _ = self.lstm(embeds)
            prediction = self.dense(lstm_out)
            # softmax is used to show probability.
            prediction = self.softmax(prediction)
        return lstm_out, prediction
